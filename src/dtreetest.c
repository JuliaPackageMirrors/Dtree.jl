...
void usage()
{
    if (my_rank == 0)
        fprintf(stderr, "Usage:\n"
                        ...
                        "            <first> <rest> <mindist> [<fanout=2048>]\n");
    MPI_Finalize();
    exit(-1);
}
...
{
    int                 i, j, min_distrib, fan_out = 2048;
    ...
    dtree_create(fan_out, num_work_items, can_parent,
            node_mul, first, rest, min_distrib, &scheduler);

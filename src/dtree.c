...
/*  build_tree() -- builds a tree of num_ranks nodes with height either
      2 or 3, depending on fan_out. The maximum number of nodes possible
      are leaf nodes.
 */
static void build_tree(dtree_t *dt, int fan_out, int can_parent)
{
    ...
    int num_parents = 0;
    div_t result = div(dt->num_ranks, fan_out);
    ...
    /* other nodes figure out their parents (and maybe children) */
        ...
        int ccount = dt->my_rank - pcount;

        /* balance children among parents */
        result = div((dt->num_ranks - num_parents), (num_parents - 1));
            ...
            dtree_t **dt_)
{
    ...
    *dt_ = NULL;
    ...
    MPI_Comm_size(MPI_COMM_WORLD, &dt->num_ranks);
    MPI_Comm_rank(MPI_COMM_WORLD, &dt->my_rank);
    ...
    if (dt->num_children) {
        printf("[%04d] parent=[%04d], #children=%d (",
               dt->my_rank, dt->parent, dt->num_children);
